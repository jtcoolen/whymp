                         ...
                         uint64_t * scratch)
{
  ...
  if (n == 1) {
    ...
    *np = *scratch;
    ...
    q = o;
    if (!(q == UINT64_C(0))) {
      wmpn_sub_in_place(npqt, h, spl, h);
    }
    IGNORE2(np, npqt);
    npl = np + l;
    wmpn_tdiv_qr_in_place(scratch, npl, n, spl, h);
    sl = scratch[l];
    q = q + sl;
    sh = *scratch;
    c = (int64_t)(sh % UINT64_C(2));
    wmpn_rshift(sp, scratch, l, UINT64_C(1));
    st = sp[l - 1];
    ql = q << ((64) - UINT64_C(1));
    qh = q >> UINT64_C(1);
    sp[l - 1] = st + ql;
    q = qh;
    if (!(c == INT64_C(0))) {
      cqt = wmpn_add_in_place(npl, h, spl, h);
      c = (int64_t)cqt;
    }
    IGNORE2(np, npl);
    npn = np + n;
    ...
    ll = 2 * l;
    bo = wmpn_sub_in_place(np, ll, npn, ll);
    b = q + bo;
    if (l == h) {
      c = c - (int64_t)b;
    } else {
...
int32_t wmpn_sqrtrem(uint64_t * sp, uint64_t * rp, uint64_t * np, int32_t n)
{
  ...
  if (!(adj == 0) || !(c == UINT64_C(0))) {
    ...
    *tp = UINT64_C(0);
    tpa = tp + adj;
    if (!(c == UINT64_C(0))) {
      wmpn_lshift(tpa, np, n, UINT64_C(2) * c);
    } else {
      i = 0;
      xp = np + 0;
      rp1 = tpa + 0;
